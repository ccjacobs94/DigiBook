...
- **Interactive VBR Duration Repair:** The web player automatically detects duration misalignment for Variable Bitrate (VBR) files by comparing the saved playback position against the loaded audio duration. If a mismatch is found, it displays a warning and allows users to trigger a server-side ffmpeg repair process with a single click, rewriting the Xing/VBR headers without needing to redigitize the entire book.
...
- **Cross-Device Sync:** Syncs playback progress via a backend SQLite database (`data/metadata.db`) so users can seamlessly resume listening across multiple devices without losing their place.
- **Docker Persistence:** Ensures that audiobooks (`/library`) and their related metadata/progress data (`/data`) persist across server and container restarts through Docker volumes.
- **Distributed Ripping and Remote Library Uploads:** Decouples the ripping process to allow standalone execution via `ripper.py` on client machines and introduces a remote upload interface in the web dashboard for importing entire directories of existing/pre-ripped digital audiobooks.
