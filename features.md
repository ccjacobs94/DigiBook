...
- **Automatic Variable Bitrate (VBR) Duration Correction:** Automatically fixes the VBR header after merging ripped disks, ensuring that media players correctly calculate and display the full duration of the audiobook without truncated "guesses".
- **Frontend VBR Misalignment Check & Repair:** The in-browser player detects if a saved playback position exceeds the detected file duration (indicative of a corrupted VBR header) and displays a warning to the user. A "Fix File Now" button allows users to trigger a server-side remux to repair the file's duration immediately without transcoding.
